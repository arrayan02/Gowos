...
class Pelanggan extends CI_Controller
{
    ...
    public function transaksi()
    {
        ...
        if ($this->session->userdata('is_login') == FALSE) {
            ...
            $data['transaksi'] = $this->db->query("SELECT tw.order_id, tw.gross_amount, tw.payment_type, tw.transaction_time, tw.bank, tw.va_number,
                tw.pdf_url, tw.status_code, tw.status ,tw.kendaraan,tw.pilihan_pencucian, tw.tanggal_pesan, tw.id_pelanggan, tw.nama, tw.alamat, tw.email, tw.no_hp,
    ...
    public function invoice($order_id)
    {
        ...
        if ($this->session->userdata('is_login') == FALSE) {
            ...
            $data['transaksi'] = $this->db->query("SELECT tw.order_id, tw.gross_amount, tw.payment_type, tw.transaction_time, tw.bank, tw.va_number,
                tw.pdf_url, tw.status_code, tw.kendaraan,tw.pilihan_pencucian, tw.tanggal_pesan, tw.id_pelanggan, tw.nama, tw.alamat, tw.email, tw.no_hp,
                tw.id_tempat_cuci, w.nama nama_usaha, w.foto1 foto1
                FROM transaksi tw 
                JOIN tempat_cuci w 
                ON tw.id_tempat_cuci = w.id 
    ...
    public function token()
    {
        ...
        $pilihan_pencucian = $this->input->post('pilihan_pencucian');
    ...
    public function finish()
    {
        ...
        $id_tempat_cuci = $this->input->post('id_tempat_cuci');
        $kendaraan = $this->input->post('kendaraan');
        $pilihan_pencucian = $this->input->post('Pilihan_Cucian');
        ...
        $data = [
            ...
            'kendaraan' => $kendaraan,
            'pilihan_pencucian' => $pilihan_pencucian,
